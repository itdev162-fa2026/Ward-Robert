...
Validates the request - Ensures email is valid and cart has items

Fetches products from database - Gets current prices (security: never trust client)

Creates Stripe line items - Formats product data for Stripe API

Converts prices to cents - Stripe requires amounts in cents ($25.99 becomes 2599)

Creates order in database - Status starts as "Pending"

Creates Stripe Checkout Session - Generates a payment page URL

Links order to session - Saves session ID for later lookup

Returns session URL - Client will redirect user here to pay

Key Security Points:

Prices always come from database, not from client request
Products are validated to exist before creating session
Order is created BEFORE payment (we track pending orders)
